...
https://github.com/FredTheBarber/VndbClient";
        ...
        private async void UpdateTagsTraitsStatsClick(object sender, EventArgs e)
        {
            ...
            if (!result) return;
            await Conn.UpdateTagsTraitsStats(tieredVns.AllVns);
            ReloadListsFromDb();
            LoadVNListToGui();
        ...
        private async void UpdateAllDataClick(object sender, EventArgs e)
        {
            ...
            if (!result) return;
            await Conn.GetMultipleVN(tieredVns.AllVns, true);
            ReloadListsFromDb();
            LoadVNListToGui();
...
{fpTitleCount} Titles by Favorite Producers.";
        ...
        private async Task UpdateAllDataSkipLimit()
        {
            ...
            if (!result) return;
            await Conn.GetMultipleVN(LocalDatabase.VNList.Select(t => t.VNID).ToArray(), true);
            ReloadListsFromDb();
            LoadVNListToGui();
        ...
        private async Task UpdateTagsTraitsStatsSkipLimit()
        {
            ...
            if (!result) return;
            await Conn.UpdateTagsTraitsStats(LocalDatabase.VNList.Select(t => t.VNID));
            ReloadListsFromDb();
            LoadVNListToGui();
...
be displayed by clicking the User Related Titles (URT) filter.",
        ...
        private async Task GetRemainingTitles()
        {
            ...
            await Conn.GetMultipleVN(unfetchedTitles.ToArray(), false);
            ReloadListsFromDb();
        }
